...
void EnP2dummy_PlayerPostSpawnSetup(Actor* thisx, PlayState* play);
void EnP2dummy_PlayerUpdate(Actor* thisx, PlayState* play);
void EnP2dummy_PlayerDraw(Actor* thisx, PlayState* play);
void EnP2dummy_PlayerDestroy(Actor* thisx, PlayState* play);
...
static void EnP2dummy_PlayerSetAnim(Player* player, PlayState* play, s32 moveState) {
    LinkAnimationHeader* anim;
    f32 playSpeed;

    if (moveState == P2DUMMY_MOVE_RUN) {
        anim = &gPlayerAnim_link_normal_run;
        playSpeed = player->actor.speed / 6.0f;
    } else if (moveState == P2DUMMY_MOVE_WALK) {
...
void EnP2dummy_PlayerPostSpawnSetup(Actor* thisx, PlayState* play) {
    ...
    thisx->flags &= ~(ACTOR_FLAG_HOSTILE | ACTOR_FLAG_TALK | ACTOR_FLAG_ATTENTION_ENABLED);
    thisx->room = play->roomCtx.curRoom.num;
...
void EnP2dummy_PlayerUpdate(Actor* thisx, PlayState* play) {
    ...
    if (!(thisx->bgCheckFlags & BGCHECKFLAG_GROUND)) {
        moveState = P2DUMMY_MOVE_AIR;
    } else if (thisx->speed < 0.2f) {
        moveState = P2DUMMY_MOVE_IDLE;
    } else if (thisx->speed < 4.0f) {
        moveState = P2DUMMY_MOVE_WALK;
    } else {
        moveState = P2DUMMY_MOVE_RUN;
    }

    EnP2dummy_PlayerSetAnim(player, play, moveState);
